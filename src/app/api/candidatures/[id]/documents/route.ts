...
export async function POST(requete: NextRequest, contexte: Contexte) {
  ...
  try {
    ...
    const document = await prisma.documentCandidature.create({ data: { candidatureId: id, type: type as TypeDocument, nomFichier: fichier.name, cheminFichier: blob.pathname, typeMime: fichier.type, tailleOctets: fichier.size } });
