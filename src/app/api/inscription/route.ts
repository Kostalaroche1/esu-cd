...
export async function GET(){const donnees=await prisma.etablissement.findMany({where:{estActif:true},select:{id:true,nom:true,sigle:true},orderBy:{nom:"asc"}});return NextResponse.json({succes:true,donnees});}
