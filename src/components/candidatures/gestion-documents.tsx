...
type DocumentCandidature = { id: string; type: TypeDocumentMetier; nomFichier: string; estValide: boolean; commentaire: string | null; statutVerification?: EtatDocument; motifVerification?: string | null; verifieLe?: string | null; verifiePar?: { nomComplet: string } | null; typeMime: string | null; tailleOctets: number | null; creeLe: string };
    ...
    <Dialog.Viewport className="fixed inset-0 z-[90] flex items-end justify-center sm:items-center sm:p-4">
      <Dialog.Popup className="flex max-h-[95vh] w-full flex-col overflow-hidden rounded-t-2xl bg-white shadow-2xl sm:max-w-4xl sm:rounded-2xl">
        ...
        <div className="min-h-0 overflow-y-auto divide-y">{documents.length === 0 ? <p className="p-10 text-center text-slate-500">Aucun document téléversé.</p> : documents.map(document => {
          const verification = lireVerification(document.estValide, document.commentaire, document.statutVerification, document.motifVerification);
