...
export function GestionModule({ module }: { module: NomModule }) {
  ...
  const roleUtilisateurSelectionne = formulaire.watch("role");
  ...
  const champsVisibles = useMemo(() => {
    if (utilisateur.role !== "ETUDIANT" || module !== "candidatures") return interfaceModule.champs;
    return interfaceModule.champs
      .filter(champ => ["appelId", "motivation", "statut"].includes(champ.nom))
      .map(champ => champ.nom === "statut" ? { ...champ, options: ["BROUILLON", "SOUMISE", "ANNULEE"] } : champ);
  }, [interfaceModule, module, utilisateur.role]);
