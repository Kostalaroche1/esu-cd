...
import { ChevronLeft, ChevronRight, FileText, MoreHorizontal, Pencil, Plus, Search, Trash2, X } from "lucide-react";
...
export function GestionModule({ module }: { module: NomModule }) {
  ...
  const [candidatureDocuments, setCandidatureDocuments] = useState<Ligne | null>(null);
  ...
  const champsVisibles = useMemo(() => utilisateur.role === "ETUDIANT" && module === "candidatures" ? interfaceModule.champs.filter(champ => ["appelId", "motivation"].includes(champ.nom)) : interfaceModule.champs, [interfaceModule, module, utilisateur.role]);
  ...
  function ouvrirModification(ligne: Ligne) {
    setSelection(ligne);
    formulaire.reset(Object.fromEntries(champsVisibles.map(champ => [champ.nom, valeurFormulaire(ligne, champ)])) as Valeurs);
    setOuvert(true);
  }
    ...
    <section className="overflow-hidden rounded-2xl border bg-white shadow-sm">
      ...
      <div className="overflow-x-auto">
        <table className="min-w-[760px] w-full text-sm"><thead className="bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-500"><tr>{interfaceModule.colonnes.map(c => <th className="px-4 py-3" key={c.cle}>{c.libelle}</th>)}<th className="px-4 py-3 text-right">Actions</th></tr></thead>
          <tbody>{chargement ? <tr><td colSpan={interfaceModule.colonnes.length + 1} className="p-10 text-center text-slate-500">Chargement des données…</td></tr> : lignes.length === 0 ? <tr><td colSpan={interfaceModule.colonnes.length + 1} className="p-10 text-center text-slate-500">Aucun enregistrement trouvé.</td></tr> : lignes.map(ligne => <tr key={String(ligne.id)} className="border-t hover:bg-slate-50/70">{interfaceModule.colonnes.map(c => <td className="max-w-xs px-4 py-3" key={c.cle}>{module === "candidatures" && c.cle === "_count.documents" ? <button onClick={() => setCandidatureDocuments(ligne)} className="rounded-lg px-2 py-1 font-medium text-blue-700 hover:bg-blue-50">{afficher(lire(ligne, c.cle))} document(s)</button> : afficher(lire(ligne, c.cle))}</td>)}<td className="px-4 py-3 text-right"><Menu.Root><Menu.Trigger className="inline-flex rounded-lg border p-2 hover:bg-white" aria-label="Menu Actions"><MoreHorizontal className="size-4"/></Menu.Trigger><Menu.Portal><Menu.Positioner side="bottom" align="end" sideOffset={6} className="z-[80]"><Menu.Popup className="w-44 rounded-xl border bg-white p-1 text-left shadow-xl outline-none">{module === "candidatures" && <Menu.Item onClick={() => setCandidatureDocuments(ligne)} className="flex cursor-pointer items-center gap-2 rounded-lg px-3 py-2 outline-none hover:bg-slate-100 focus:bg-slate-100"><FileText className="size-4"/>Documents</Menu.Item>} {peutModifier && <Menu.Item onClick={() => ouvrirModification(ligne)} className="flex cursor-pointer items-center gap-2 rounded-lg px-3 py-2 outline-none hover:bg-slate-100 focus:bg-slate-100"><Pencil className="size-4"/>Modifier</Menu.Item>}{estSuperAdministrateur && <Menu.Item onClick={() => void supprimer(ligne)} className="flex cursor-pointer items-center gap-2 rounded-lg px-3 py-2 text-red-700 outline-none hover:bg-red-50 focus:bg-red-50"><Trash2 className="size-4"/>Supprimer</Menu.Item>}</Menu.Popup></Menu.Positioner></Menu.Portal></Menu.Root></td></tr>)}</tbody>
        </table>
      </div>
