    ...
    <section className="overflow-hidden rounded-2xl border bg-white shadow-sm">
      ...
      <div className="overflow-x-auto">
        <table className="min-w-[760px] w-full text-sm"><thead className="bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-500"><tr>{interfaceModule.colonnes.map(c => <th className="px-4 py-3" key={c.cle}>{c.libelle}</th>)}<th className="px-4 py-3 text-right">Actions</th></tr></thead>
          <tbody>{chargement ? <tr><td colSpan={interfaceModule.colonnes.length + 1} className="p-10 text-center text-slate-500">Chargement des données…</td></tr> : lignes.length === 0 ? <tr><td colSpan={interfaceModule.colonnes.length + 1} className="p-10 text-center text-slate-500">Aucun enregistrement trouvé.</td></tr> : lignes.map(ligne => <tr key={String(ligne.id)} className="border-t hover:bg-slate-50/70">{interfaceModule.colonnes.map(c => <td className="max-w-xs px-4 py-3" key={c.cle}>{afficher(lire(ligne, c.cle))}</td>)}<td className="px-4 py-3 text-right"><details className="relative inline-block"><summary className="list-none cursor-pointer rounded-lg border p-2 hover:bg-white" aria-label="Menu Actions"><MoreHorizontal className="size-4"/></summary><div className="absolute right-0 z-20 mt-1 w-40 rounded-xl border bg-white p-1 text-left shadow-xl">{module === "candidatures" && <GestionDocuments candidatureId={String(ligne.id)} reference={String(ligne.reference)}/>} {peutModifier && <button onClick={() => ouvrirModification(ligne)} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 hover:bg-slate-100"><Pencil className="size-4"/>Modifier</button>}{estSuperAdministrateur && <button onClick={() => void supprimer(ligne)} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-red-700 hover:bg-red-50"><Trash2 className="size-4"/>Supprimer</button>}</div></details></td></tr>)}</tbody>
        </table>
      </div>
