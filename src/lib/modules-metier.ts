...
export const schemasModules = {
  ...
  programmes: z.object({ code: texte, nom: texte, description: optionnel, organismeFinanceur: optionnel, montantMaximum: nombre.optional().nullable(), devise: texte.default("USD"), niveauCible: optionnel, domaineCible: optionnel, estActif: booleen }),
  appels: z.object({ reference: texte, titre: texte, description: optionnel, dateOuverture: date, dateCloture: date, nombrePlaces: z.coerce.number().int().positive().optional().nullable(), criteresEligibilite: optionnel, estPublie: booleen, programmeId: texte }).refine(v => v.dateCloture >= v.dateOuverture, { message: "La clôture doit suivre l’ouverture.", path: ["dateCloture"] }),
  candidatures: z.object({ reference: texte, motivation: optionnel, statut: z.enum(["BROUILLON", "SOUMISE", "EN_VERIFICATION", "ELIGIBLE", "NON_ELIGIBLE", "EN_EVALUATION", "RETENUE", "REJETEE", "ANNULEE"]).optional(), scoreFinal: nombre.min(0).max(100).optional().nullable(), commentaire: optionnel, etudiantId: texte, appelId: texte }),
  evaluations: z.object({ note: nombre.min(0).max(100).optional().nullable(), decision: z.enum(["FAVORABLE", "DEFAVORABLE", "A_REVOIR"]), commentaire: optionnel, candidatureId: texte, evaluateurId: texte }),
  ...
  paiements: z.object({ reference: texte, periode: texte, montant: nombre.positive(), devise: texte.default("USD"), datePrevue: date, datePaiement: z.coerce.date().optional().nullable(), statut: z.enum(["PREVU", "EFFECTUE", "ECHEC", "ANNULE"]).optional(), modePaiement: optionnel, preuveUrl: optionnel, attributionId: texte }),
  renouvellements: z.object({ anneeAcademique: texte, decision: z.enum(["RENOUVELEE", "SUSPENDUE", "CLOTUREE", "ANNULEE"]), commentaire: optionnel, attributionId: texte }),
